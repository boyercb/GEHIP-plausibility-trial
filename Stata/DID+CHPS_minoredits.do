...
orth_out sex multiple cduration bspace cparity cm_age if period == 1, by(treatment) pcompare count vce(cluster deacode)
orth_out sex multiple cduration bspace cparity cm_age if period == 0, by(treatment) pcompare count vce(cluster deacode)
...
restore
estat phtest, detail
outreg2 using "`path'/DiD_sensitivity.xml", excel replace sideway ci eform addstat("No. Subjects", e(N_sub), "No. Clusters", e(N_clust), "Log-likelihood", e(ll), "Chi-squared", e(chi2), "df", e(df_m))
stcox i.treatment i.period i.GEHIP if nonmissing, cluster(deacode) efron
outreg2 using "`path'/DiD_sensitivity.xml", excel append sideway ci eform addstat("No. Subjects", e(N_sub), "No. Clusters", e(N_clust), "Log-likelihood", e(ll), "Chi-squared", e(chi2), "df", e(df_m))
...
outreg2 using "`path'/DiD_sensitivity.xml", excel append sideway ci eform
...
logit neonate i.period i.treatment i.GEHIP, cluster(deacode) or
outreg2 using "`fout'", excel replace sideway ci eform addstat("No. Subjects", e(N_sub), "No. Clusters", e(N_clust), "Log-likelihood", e(ll), "Chi-squared", e(chi2), "df", e(df_m))
